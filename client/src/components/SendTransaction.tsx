...
const sendTransactionFormSchema = z.object({
  sender: z.string()
    ...
    .regex(WalletIDPattern, 'Sender address must be a 42 hexadecimal string.'),
  recipient: z.string()
  .nonempty('Recipient address is required.')
  .regex(WalletIDPattern, 'Recipient address must be a 42 hexadecimal string.'),
  amount: z.number({
...
const SendTransaction: React.FC<ISendTransactionProps> = ({ senderAddress }) => {
  ...
  const onSubmit: SubmitHandler<FormData> = ({ recipient, amount }) => {
    handleDispatch({ recipient, amount })
  };

  const handleDispatch = useCallback(({ recipient, amount }: SendTransactionSagaPayload) => {
    dispatch({
      type: Actions.SendTransaction,
      payload: { recipient, amount: amount.toString() }
    });
  }, [dispatch]);
          ...
          <div className="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-100 transition-all w-full m-3 mx-auto flex flex-col h-full items-center justify-center">
            <div className="bg-white border shadow-sm rounded-xl w-modal">
              <div className="flex justify-between items-center py-3 px-4 border-b">
                ...
                <button type="button" onClick={ () => reset() } className="hs-dropdown-toggle inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white transition-all text-sm" data-hs-overlay="#hs-basic-modal">
              ...
              <div className="p-4 overflow-y-auto">
                ...
                <input type="text" id="input-sender" className="opacity-70 pointer-events-none py-3 px-4 block bg-gray-50 border-gray-800 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 w-full" placeholder="Sender Address (Autocompleted)" {...register("sender")} disabled/>
                { errors.sender && <span className='text-red-500'>{errors.sender.message}</span> }
                ...
                { errors.recipient && <span className='text-red-500'>{errors.recipient.message}</span> }
                <label htmlFor="input-amount" className="block text-sm font-bold my-2">Amount:</label>
                <input type="number" id="input-amount" className="opacity-70 py-3 px-4 block bg-gray-50 border-gray-800 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 w-full" placeholder="Amount" {...register("amount", { valueAsNumber: true })} />
                { errors.amount && <span className='text-red-500'>{errors.amount.message}</span> }
              </div>
              <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
                <button type="button" onClick={ () => reset() } className="hs-dropdown-toggle py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border font-medium bg-white text-gray-700 shadow-sm align-middle hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:ring-blue-600 transition-all text-sm" data-hs-overlay="#hs-basic-modal">
                  Close
                </button>
                <button type="submit" className="hs-dropdown-toggle py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white disabled:bg-blue-200 disabled:cursor-not-allowed hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm">
                  Send
                </button>
              </div>
            </div>
